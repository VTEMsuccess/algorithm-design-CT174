...
// Moi do vat co 1 so luong duy nhat tai moi loai do vat
...
void Nhanh_Can(int i, float *TGT, float *CT, float *V, float *GLNTT, int x[], DoVat *dsdv, int n){
	...
	// yk =  *V/dsdv[i].TL; cai ba lo 1 khong gioi han so do vat
	
	yk = min(1, *V/dsdv[i].TL); // cai ba lo 3 chi co so luong duy nhat 1 tai moi do vat
	 
	// yk = min(dsdv[i].SL, *V/dsdv[i].TL);	 cai ba lo 2 so luong do vat cho truoc xac dinh
