...
void in_ma_tran(canh a[][size], int n){
	...
	for(i=0; i<n; i++){
		for(j=0; j<n; j++)
			printf(" %c%c =%5.2f  ",a[i][j].dau+97, a[i][j].cuoi+97, a[i][j].do_dai);	//ky tu in thuong a trong ban ma ASCII
		printf("\n");
	}
}
...
void in_PA(canh PA[], int n){
	...
	for(i=0; i<n; i++){
		sum+= PA[i].do_dai;
		printf(" Canh %c%c = %5.2f\n", PA[i].dau+97, PA[i].cuoi+97, PA[i].do_dai);	//ky tu in thuong chu a trong ban ma ASCII
    } 
...
int main(){
	...
	while (1){
		...
		if(tpxp <97) tpxp +=32; //doi chu Hoa thanh thuong 
